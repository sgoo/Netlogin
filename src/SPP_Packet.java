...
public class SPP_Packet {
	...
	public SPP_Packet(String paramString, int paramInt) throws IOException,
			UnknownHostException {
		this.passwdSocket = new Socket(paramString, paramInt);
		this.os = new DataOutputStream(new BufferedOutputStream(
				this.passwdSocket.getOutputStream()));
		this.is = new DataInputStream(new BufferedInputStream(
				this.passwdSocket.getInputStream()));
	}

	public SPP_Packet() throws IOException, UnknownHostException {
	...
	public void SendPacket(int paramInt1, int paramInt2, byte[] paramArrayOfByte)
		...
		try {
			...
			throw new IOException("SendPacket: 1"
					+ localIOException1.getMessage());
		}
		try {
			this.os.writeInt(paramInt1);
		} catch (IOException localIOException2) {
			throw new IOException("SendPacket: 2"
					+ localIOException2.getMessage());
		}
		try {
			this.os.writeInt(paramInt2);
		} catch (IOException localIOException3) {
			throw new IOException("SendPacket: 3"
					+ localIOException3.getMessage());
		}
		try {
			this.os.write(paramArrayOfByte, 0, paramArrayOfByte.length);
		} catch (IOException localIOException4) {
			throw new IOException("SendPacket: 4"
					+ localIOException4.getMessage());
		}
		try {
			this.os.flush();
		} catch (IOException localIOException5) {
			throw new IOException("SendPacket: 5"
					+ localIOException5.getMessage());
		}
	}

	public ReadResult ReadPacket(int paramInt1, int paramInt2,
		...
		if (this.Last_Read_length < 0)
			throw new IOException("ReadPacket 1: data length "
					+ this.Last_Read_length + " < 0");
		if (this.Last_Read_length > paramArrayOfByte.length)
			throw new IOException("ReadPacket 2: data length "
					+ this.Last_Read_length + " > buffer length");
		...
		while ((i < this.Last_Read_length) && (j != -1)) {
			try {
				j = this.is
						.read(paramArrayOfByte, i, this.Last_Read_length - i);
			} catch (IOException localIOException) {
				throw new IOException("ReadPacket 5: "
 ...
 * Name: SPP_Packet JD-Core Version: 0.6.2
 */